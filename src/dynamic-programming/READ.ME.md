...
Dynamic Programming is a method for solving a complex problem by breaking it down into a collection of simpler subproblems just once, and storing their solutions using a memory-based data structure.

Dynamic prommaing in itself isn't difficult. The difficult part is figuring out how to turn the problem into subproblems so that you can apply dynamic programming to it.

# General Indicators

Type of problems: DP is commonly used for optimization (finding the minimum or maximum), counting problems, and certain "Yes/No" questions (can something be done given the constraints).
...
1. **Are there overlapping subproblems?**
    ...
    - Does solving the problem involve solving the same smaller problem multiple times?

2. **Is there an optimal substructure?**

    - Can a solution to the whole problem be constructed from the solutions of the subproblems?

3. **Can the problem be broken down into smaller, simpler subproblems?**

    - Is it possible to divide the problem into smaller, simpler subproblems?

4. **Is memoization or tabulation applicable?**

    - Can the results of subproblems be stored and efficiently retrieved to avaoid redundant calculations?

5. **Are there multiple ways to reach a solution?**

    - Does the problem involve making a series of choices at each step to reach an overall optimal soluton

6. **Is the problem an optimization problem?**

    - Does the probleminvole making a series of choices at each step to reach an overall optimal solution?
...
4. **What is the Reccurence Relation?**
...
5. **How to Store Solutions to Subproblems?**
    ...
    - Will you use memoization (top-down with recursion) or tabulation (bottom-up iteration)? Decide on the data structure (array, matrix, hasmap) to store these solutions.
...
7. **How to Construct the Solution to the Original Problem?**
    ...
    - Once subproblems are solved, how do you combine their solutions to get the solution to the orignal problem?

8. **Are there Initialization Conditions?** 

    - For tabulation, how do you combine their solutions to get the solution to the original problem?
...
To visualize how this is working, let's draw a tree to the recursive calls:
...
```
  ...
  /    \  /   \   /   \ /    \ /   \  /    \ / \  /  \
(4, 0) (3, 1) (3, 1) (2, 2) (3, 1) (2, 2) (2, 2) (1, 3) (3, 1) (2, 2) (1, 3) (2, 2) (1, 3) (1, 3) (0, 4)
```
...
1. (1, 0) -> (2, 0) -> (3, 0) -> (4, 0) this is out of bounds, we return 0. `down` is 0.
