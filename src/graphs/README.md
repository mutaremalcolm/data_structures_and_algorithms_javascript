...
DFS is like a maze. You go as far as you can in one direction. If you hit a dead end, you backtrack and try another direct. You keep track of all directions you've tried, so you don't go in circles.
...
Interestingly, this is a little bit difficult. The approach is more, either you know it or you don't. It's not as intuitive as DFS.
...
No, because we loop over ther vertex's edges and enqueue all of them.

Then we dequeue the next vertex. We loop over it's edges and enqueue all of them.
...
This one is difficult. It took me a while to grasp it, and then turning in into code was hard.
...
```js
   findShortestPathDijkstra(start, end) {
    ...
    // All nodes expect the start node have a distance of Infinity
    ...
    while (unvisted.length) {
        const keyOfNodesWithSmallestDistance = this.#findNodeWithSmallestDistance(
            unvisited,
            distances
        )

        if (!keyOfNodeWithSmallestDisatnce) {
            break
        }
        ...
        edgesForNode.forEach((edge) => {
            const distanceToNeighbor =
                diestances[keyOfNodeWithSmallestDistance] + edge.weight
...
Just because I tell you the shortest distance between two nodes, doesn't mean you know how to get there. I can be like hey, the shortest distance between Limpopo and Pretoria is 350 KMs, but as you get going, you will be like, oh wait, which path exactly do i take? So you need to keep track of the previous nodes.
...
We loop over the edges of the current node. We calculate the distance to the neighbor. If the distance to the neighbor is shorter than the distance we have stored in the distance object, we update the distances object and the previous nodes object.
...
We calculate the distance of current node by getting it from distances object. This way, we make sure we're getting the sum of the weights: `const distanceToNeighbor = distances[keyOfNodeWithSmallestDistance] + edge.weight`
...
// Loop through the previous nodes object until we reavh the start node
while (currentNode !== start) {
    // Add the current node to the start path
    path.unshift(currentNode)
    currentNode = previousNodes[currentNode]
...
1. **Local Optimal Choices** A greedy algorithm makes a series of choices that seem the best at that momemnt, without considering the global context. It picks the locally optimal solution at each step with the hope of finding a global optimum.
